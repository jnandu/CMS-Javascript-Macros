 ...
 */
function DebugPrint(){
  ARVConsole(DebugString(arguments));
}
/**
 * Format an item for debugging purposes
...
function Union (range1, range2){
 var doc = DocumentApp.create('TempDoc');
 var rangeBuilder = doc.newRange();
 var rg1Len = range1.getNumRows();
 var rg1Ele = range1.getDataTable();
 //for (var i = 0; i < rg1Ele.length; i++) {
   rangeBuilder.addElement(rg1Ele);
 //}
 var rg2Ele = range1.getDataTable();
 //for (var i = 0; i < rg2Ele.length; i++) {
   rangeBuilder.addElement(rg2Ele);
 //}
 //rangeBuilder.addRange(range1);
 var name = 'temp'+range1.getA1Notation()+range2.getA1Notation();
 var newRange = rangeBuilder.build();
 doc.addNamedRange(name, newRange);
 return newRange;
}

function Union1(range1, range2) {
  var result = [];
  var length = 0;
  var i = 0;
  try {
...
function getRangeOfNamedRange(name, nRanges) {
     var rangeA1Name = nRanges[name];
     var position = get2DAddrFromA1Notation(rangeA1Name);
     ARVConsole("position="+position);
     return position;

}

function getRangeOfCheckBox(sheet, checkName){
  ARVConsole("checkName="+checkName+"checkBox2A1Not[checkName]="+checkBox2A1Not[checkName]);
  var rangeA1Name;
  if ((rangeA1Name = checkBox2A1Not[checkName])){
    var position = get2DAddrFromA1Notation(rangeA1Name);
    return position;
  }else{
